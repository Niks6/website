...
async function loadProducts() {
  try {
    ...
    showMessage("Network error loading products. Please check your connection.", "error");
  }
}
...
function renderProductsTable() {
  ...
  tbody.innerHTML = filteredProducts
    .map(
      (product) => `
        <tr>
          <td><img src="${product.imageUrl}" alt="${product.name}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 4px;" /></td>
...
async function updateStock(productId) {
  ...
  try {
    ...
    if (result.success) {
      ...
      showMessage("Failed to update stock: " + result.error, "error");
    }
  } catch (error) {
      ...
      <div class="dropdown-option" data-product-id="${
        ...
        <div class="option-content">
          <div class="option-main">
            ...
            <span class="option-wh-price">₹${(product.wholesalePrice || 0).toFixed(2)}</span>
          </div>
...
async function generateInvoice() {
  ...
  const totalElement = document.getElementById("invoice-total");
  const total = totalElement ? Number.parseFloat(totalElement.textContent) : 0;

  if (!customerName || !customerAddress || !customerMobile || total <= 0) {
    showMessage("Please fill in all customer details and add items to the invoice.", "error");
    return;
  }
...
// Load seller requests from orders collection
async function loadSellerRequests() {
  try {
    ...
    if (result.success) {
      ...
      console.log("Loaded seller requests:", sellerRequests);
      renderTabContent();
      updateDashboardStats();
      return result.data;
    } else {
    ...
    sellerRequests = [];
    showMessage("Network error loading orders. Please check your connection.", "error");
  }
}
  ...
  try {
    ...
    if (action === "accepted") {
      ...
      if (!order || !Array.isArray(order.cart)) {
        showMessage("Order not found or invalid cart data", "error");
        return;
      }
      ...
      try {
        ...
        const stockUpdateSummary = stockUpdates
          ...
          .map(
            (update) =>
              `• ${update.productName}: ${update.oldStock} → ${update.newStock} (Sold: ${update.quantity})`
          )
          .join("\n");
        ...
        showMessage(
          `Error updating stock levels: ${stockError.message}\n\nOrder will not be accepted.`,
          "error"
        );
        return;
      }
    ...
    if (result.success) {
      showMessage(`Order ${action} successfully!`, "success");
...
// View order details with modal functionality
function viewOrderDetails(orderId) {
  ...
  const cartItemsHtml =
    Array.isArray(order.cart) && order.cart.length > 0
      ? order.cart
          ...
          .join("")
      : '<tr><td colspan="7" style="text-align: center; color: #6b7280;">No items in cart</td></tr>';
  ...
  const modalHTML = `
    <div class="modal-overlay" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000;">
      <div class="modal-content" style="background: white; border-radius: 8px; max-width: 90%; max-height: 90%; overflow-y: auto; padding: 0;">
        <div class="modal-header" style="padding: 1rem; border-bottom: 1px solid #e5e7eb; display: flex; justify-content: space-between; align-items: center;">
          <h3 style="margin: 0;">Order Details - ${
            order.Status === "pending" ? "PENDING" : order.Status === "accepted" ? "ACCEPTED" : "REJECTED"
          }</h3>
          <button class="modal-close" type="button" style="background: none; border: none; font-size: 1.5rem; cursor: pointer;">&times;</button>
        </div>
...
function downloadOrderRequest(orderId) {
  ...
  showMessage("Order document downloaded successfully!", "success");
}
...
function addOffer() {
  ...
  if (!imageInput || !descInput) return;

  const file = imageInput.files[0];
  const description = descInput.value.trim();

  if (!file || !description) {
    showMessage("Please upload an image and add a description.", "error");
    return;
  }
...
function updateDashboardStats() {
  ...
  if (pendingRequestsEl) pendingRequestsEl.textContent = sellerRequests.filter((r) => r.Status === "pending").length;
  if (lowStockItemsEl) lowStockItemsEl.textContent = products.filter((p) => p.stock < 15).length;
...
// PDF Invoice Generation (requires jsPDF library)
function generateInvoicePDF(invoiceData, total) {
  if (typeof window.jspdf === 'undefined') {
    showMessage("PDF library not loaded. Please include jsPDF library.", "error");
    return;
  }
  ...
  items.forEach((item, index) => {
    ...
    const description = pro ? `${pro.company} ${pro.name}` : item.productName || "Unknown Product";
    const descriptionLines = doc.splitTextToSize(description, 95);
    doc.text(descriptionLines[0], 20, yPos);
...
function generateSampleInvoice(customerName, customerAddress, customerMobile, validItems, total) {
  ...
  const sampleData = {
    company: {
      ...
      phone: "(555) 123-4567",
      email: "info@guptaelectronics.com",
    },
    invoice: {
    ...
    taxRate: 18,
    notes:
      "Thank you for choosing Gupta Electronics! This invoice is generated for your purchase.",
  };
...
window.editProduct = (id) => showMessage(`Edit product ${id} - Feature coming soon!`, "info");
window.deleteProduct = (id) => showMessage(`Delete product ${id} - Feature coming soon!`, "info");
